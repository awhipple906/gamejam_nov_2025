...
func _check_can_attack(attack_stats : MeleeAttackStats, target : CharacterBody3D):
	if(attack_stats.has_overlapping_bodies() and !target_out_of_range(target) and !attack_stats.isOnCoolDown):
		return true
	else:
		return false
